...
impl CircuitBreaker {
    ...
    pub async fn execute_with_retry<F, R, E>(&self, mut operation: F, max_retries: u32) -> Result<R, CircuitBreakerError<E>>
    where
        ...
        loop {
            ...
            match operation().await {
                ...
                Err(error) => {
                    ...
                    debug!("Retrying operation in {:?} (attempt {}/{})", delay, retry_count + 1, max_retries);
                    sleep(delay).await;
                }
            }
        }
