...
impl Consumer {
        ...
        if !pending_items.is_empty() {
            ...
            let claim_result = self
                ...
                .xclaim(
                    ...
                    group_name,
                    waypoint_consumer,
                    Duration::from_millis(0), // Force claim immediately
                    &msg_ids,
                )
                .await;
            ...
            match claim_result {
                Ok(_) => {
                    ...
                    if pending_items.len() == BATCH_SIZE {
                            ...
                            match more_pending {
                                Ok(more_msgs) if !more_msgs.is_empty() => {
                                    ...
                                    let more_ids: Vec<String> =
                                        more_msgs.iter().map(|item| item.id.clone()).collect();

                                    let more_claim_result = self
                                        .stream
                                        .redis
                                        .xclaim(
                                            stream_key,
    ...
    async fn cleanup_old_events(&self, message_type: MessageType) {
        ...
        while !self.should_shutdown().await {
            ...
            match self.stream.trim(&stream_key, 100000).await {
                Ok(_) => {
