use crate::{
    config::StreamProcessorConfig, processor::stream::StreamProcessor, proto::HubEvent,
...
impl Consumer {
    ...
    pub async fn start(&mut self) -> Result<(), crate::redis::error::Error> {
        ...
        for (event_type, group_suffix) in self.get_stream_keys() {
            ...
            let group_name = format!("{}:{}", self.config.group_name, group_suffix);
            ...
            let consumers_per_stream = self.config.consumers_per_stream;
            for consumer_num in 0..consumers_per_stream {
                ...
                let processor = StreamProcessor {
                    ...
                    max_events_per_fetch: self.config.batch_size,
                    processing_concurrency: self.config.concurrency,
                    event_processing_timeout: Duration::from_secs(
                        self.config.event_processing_timeout_secs,
                    ),
        ...
        let event_retention_secs = self.config.event_retention_secs;
        ...
        for (event_type, _) in self.get_stream_keys() {
            ...
            tokio::spawn(async move {
                ...
                while !*cleanup_shutdown.read().await {
                    interval.tick().await;
                    if let Err(e) = cleanup_stream
                        .trim(&cleanup_key, Duration::from_secs(event_retention_secs))
                        .await
                    {
