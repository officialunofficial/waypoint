use crate::{
    ...
    proto::{Message, MessageData, self},
};
use chrono::Utc;
use rand::Rng;
...
fn generate_random_message(message_type: i32, fid: u64) -> Message {
    ...
    match message_type {
        1 => { // CastAdd
            message.data.as_mut().unwrap().body = Some(proto::message_data::Body::CastAddBody(
                proto::CastAddBody {
        ...
        3 => { // ReactionAdd
            message.data.as_mut().unwrap().body = Some(proto::message_data::Body::ReactionBody(
                proto::ReactionBody {
                    r#type: 1, // Like
                    target: Some(proto::reaction_body::Target::TargetCastId(
                        proto::CastId {
        ...
        5 => { // LinkAdd
            message.data.as_mut().unwrap().body = Some(proto::message_data::Body::LinkBody(
                proto::LinkBody {
        ...
        11 => { // UserDataAdd
            message.data.as_mut().unwrap().body = Some(proto::message_data::Body::UserDataBody(
                proto::UserDataBody {
