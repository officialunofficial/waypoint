...
pub const CAST_RETRY_STREAM: &str = "cast_retry_stream";

/// Redis stream key for dead letter queue
pub const CAST_RETRY_DEAD: &str = "cast_retry_dead";

/// Maximum retry attempts before moving to dead letter queue
pub const MAX_RETRY_ATTEMPTS: u64 = 3;
...
pub enum CastProcessingResult {
    ...
    TemporaryFailure { error: String, retry_after_seconds: u64 },
...
async fn is_parent_lookup_cached_failure(
    ...
    let exists: Result<bool, _> =
        bb8_redis::redis::cmd("EXISTS").arg(&key).query_async(&mut *conn).await;
...
async fn add_to_retry_queue(
    ...
    let retry_data = RetryData {
        cast_fid,
        cast_hash: cast_hash.map(|h| h.to_vec()),
        parent_fid,
        parent_hash: parent_hash.map(|h| h.to_vec()),
    ...
    debug!(
        "Added cast to retry queue: stream={}, attempt={}, error={}",
        stream_key, attempt, error
    );
    Ok(())
}
...
pub async fn find_root_parent_hub_with_retry<H: HubClient>(
    ...
    match find_root_parent_hub_internal(
        hub_client,
        redis,
        parent_fid,
        parent_hash,
...
pub async fn find_root_parent_hub<H: HubClient>(
    ...
    match find_root_parent_hub_internal_simple(
        hub_client,
        parent_fid,
        parent_hash,
        parent_url,
...
async fn find_root_parent_hub_internal<H: HubClient>(
                    ...
                    match &cast_body.parent {
                        Some(Parent::ParentCastId(parent_cast_id)) => {
                            ...
                            if visited.contains(&(parent_fid, parent_hash.clone())) {
                                let error = format!(
                                    "Cycle detected in cast parent chain at fid={}",
                                    parent_fid
                                );
                                warn!("{}", error);
                                return CastProcessingResult::Success(Some(RootParentInfo {
    ...
    CastProcessingResult::TemporaryFailure { error, retry_after_seconds: NEGATIVE_CACHE_TTL }
}
