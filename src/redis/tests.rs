...
mod redis_client_tests {
    ...
    async fn test_stream_key_generation() {
        ...
        // Test without shard suffix
        let key = crate::types::get_stream_key(host, stream_type, None);
        assert_eq!(key, "hub:hub.example.com:stream:casts");

        // Test with shard suffix
        let key_with_shard = crate::types::get_stream_key(host, stream_type, Some("shard_0"));
        assert_eq!(key_with_shard, "hub:hub.example.com:stream:casts:shard_0");

        // Test with empty shard suffix (should not create dangling colon)
        let key_empty_shard = crate::types::get_stream_key(host, stream_type, Some(""));
        assert_eq!(key_empty_shard, "hub:hub.example.com:stream:casts");
        assert!(!key_empty_shard.ends_with(':'), "Key should not have dangling colon");

        // Test with port in host (should be stripped)
        let host_with_port = "hub.example.com:2283";
        let key_no_port = crate::types::get_stream_key(host_with_port, stream_type, None);
        assert_eq!(key_no_port, "hub:hub.example.com:stream:casts");

        // Test with port and shard
        let key_port_shard =
            crate::types::get_stream_key(host_with_port, stream_type, Some("shard_1"));
        assert_eq!(key_port_shard, "hub:hub.example.com:stream:casts:shard_1");
    }
}
